...
import { flowsourcePlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [flowsourcePlugin],
    });

    await request(server).get('/api/flowsource/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/flowsource/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/flowsource/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/flowsource/todos/${createdTodoItem.id}`)
